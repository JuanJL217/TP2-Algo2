...
bool movimientos_agregar(movimientos_t *movimientos, char *clave,
			 void (*funcion)(char *, size_t *, size_t *, size_t,
					 size_t))
{
	if (!movimientos || !clave || !funcion)
		return false;
	funcion_movimiento_t *movimiento =
...
bool movimiento_realizar(movimientos_t *movimientos, char *movimiento_externo,
			 char *movimiendo_realizar, size_t *fila,
			 size_t *columna, size_t fila_limite,
			 size_t columna_limite)
{
	if (!movimientos || !fila || !columna) {
		return false;
	}
	funcion_movimiento_t *func = (funcion_movimiento_t *)hash_buscar(
		movimientos, movimiendo_realizar);
	if (!func)
		return false;
	func->funcion_movimiento(movimiento_externo, fila, columna, fila_limite,
				 columna_limite);
	return true;
}
